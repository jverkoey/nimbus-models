...
public typealias Action = BoolTargetSignature
...
public struct ObjectActions {
  ...
  func performTapAction(object: NSObject, indexPath: NSIndexPath) -> Bool? {
    ...
    if let tap = tap {
      if tap(object: object, indexPath: indexPath) {
        shouldDeselect = true
      }
    }
    if let tapSelector = tapSelector {
      if tapSelector.performAction(object, indexPath: indexPath)! {
        shouldDeselect = true
      }
    }
...
public class Actions : NSObject {
  var objectToAction: [Int: ObjectActions] = [:]
  var classToAction: [String: ObjectActions] = [:]
}
